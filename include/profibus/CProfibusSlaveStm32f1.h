...
{
protected:
  void configTimer(void) override
  {
    MX_TIM2_Init();
  ...
  void waitForActivTransmission(void) override
  {
    while (__HAL_UART_GET_FLAG(&huart3, UART_FLAG_TXE) != SET)
      ;
  }

  void activateTxInterrupt(void) override
  {
    clearTxFlag();
  ...
  void configRs485Pin(void) override
  {
    HAL_GPIO_WritePin(GPIOB, PB_TX_EN_Pin, GPIO_PIN_RESET);
    HAL_GPIO_WritePin(GPIOB, PB_RX_EN_Pin, GPIO_PIN_SET);
  } // nothing to do

  void TxRs485Enable(void) override
  {
    HAL_GPIO_WritePin(GPIOB, PB_RX_EN_Pin, GPIO_PIN_SET);
    HAL_GPIO_WritePin(GPIOB, PB_TX_EN_Pin, GPIO_PIN_SET);
  }

  void TxRs485Disable(void) override
  {
    HAL_GPIO_WritePin(GPIOB, PB_TX_EN_Pin, GPIO_PIN_RESET);
    HAL_GPIO_WritePin(GPIOB, PB_RX_EN_Pin, GPIO_PIN_RESET);
  }

  void RxRs485Enable(void) override
  {
    HAL_GPIO_WritePin(GPIOB, PB_TX_EN_Pin, GPIO_PIN_RESET);
    HAL_GPIO_WritePin(GPIOB, PB_RX_EN_Pin, GPIO_PIN_RESET);
  }
