    ...
    DataHandling: DataHandlingInterface,
{
    pub fn new(
        ...
        let counter_frequency = hw_interface.get_timer_frequency();
        let baudrate = hw_interface.get_baudrate();

        let timeout_max_syn_time_in_us = (33 * counter_frequency) / baudrate; // 33 TBit = TSYN
        let timeout_max_rx_time_in_us = (15 * counter_frequency) / baudrate;
        let timeout_max_tx_time_in_us = (15 * counter_frequency) / baudrate;
        let timeout_max_sdr_time_in_us = (15 * counter_frequency) / baudrate; // 15 Tbit = TSDR
    ...
    fn transmit(&mut self) {
        ...
        if 0 != self.min_tsdr {
            ...
            self.timer_timeout_in_us = (counter_frequency * u32::from(self.min_tsdr))
                / baudrate
                / 2u32;
            self.hw_interface.run_timer(self.timer_timeout_in_us);
        } else {
